...
class RestProxyTest extends FreeSpec
  ...
  val restProxyUrl = "localhost:8082"
  ...
  val clusterDescription: DescribeClusterResult = adminClient.describeCluster()
  ...
  "produce" - {
    ...
    "produce single json message" in {
      ...
      produceResponse.body fold(e => logger.error(s"failed: $e"), r => logger.info(s"success: $r"))
    }

    "produce json message batch" in {

      val msgs = (1 to 10000).toList map { _: Int => Message(Random.alphanumeric.take(5).mkString, Random.alphanumeric.take(100).mkString) }
    ...
    "produce parallel json message batches - ordered" in {
      ...
      val msgs = (1 to 100000).toList map { _: Int => Message(Random.alphanumeric.take(5).mkString, Random.alphanumeric.take(100).mkString) }
      val batches: Iterable[Records] = msgs.sliding(100, 100).to(Iterable).map(Records(_))


      // time for create and dispatch requests, receiving and decoding responses 5317.291518ms
      val produceResponses: List[Response[Either[ResponseError[Error], ProduceResponse]]] = timed("create and dispatch requests, receiving and decoding responses") {

        val sent: Iterable[Task[Response[Either[ResponseError[Error], ProduceResponse]]]] = batches map { b => reqBase.body(b).send() }
        val par: Iterable[Task[Iterable[Response[Either[ResponseError[Error], ProduceResponse]]]]] = sent.sliding(8, 8).to(Iterable).map(b => Task.gather(b))
    ...
    "produce parallel json message batches - unordered" in {
      ...
      val produceResponses: List[Response[Either[ResponseError[Error], ProduceResponse]]] = timed("create and dispatch requests, receiving and decoding responses") {
        ...
        //val sent: Iterable[Task[Response[Either[ResponseError[Error], ProduceResponse]]]] = batches map { b => reqBase.body(b).send() }
        val p: Observable[Response[Either[ResponseError[Error], ProduceResponse]]] = Observable.fromIterable(batches).mapParallelUnordered(parallelism = 8) { b => reqBase.body(b).send()
        }
